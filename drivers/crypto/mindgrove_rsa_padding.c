...
/* SHA256 digest info */
static uint8_t sha256_digestinfo[] = {0x30, 0x31, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01,
				      0x65, 0x03, 0x04, 0x02, 0x01, 0x05, 0x00, 0x04, 0x20};

/* Helper function to generate mask of data block, and seed */
static uint16_t Mask_Generation(uint8_t *mask, size_t maskLen, const uint8_t *seed, size_t seedLen)
{
	uint8_t result = 0;
				  ...
				  size_t input_length)
{
	...
	/* Check DigestInfo */
	if (memcmp(signature + index, sha256_digestinfo, sizeof(sha256_digestinfo)) != 0) {
		return EINVAL;
	}
			    ...
			    size_t input_length, uint8_t *label, size_t label_length)
{
	...
	padding_string_len = data_block_length - input_length - SHA256_HASH_LEN - 1U;
	...
	result = Mask_Generation(seedMask, SHA256_HASH_LEN, maskedDB, data_block_length);
	if (result != SUCCESS) {
		return result;
	}
			 ...
			 uint8_t *salt, size_t salt_length, uint8_t use_provided_salt)
{
	...
	/* Hashing M_prime */
	result =
		SHA256_Single_Run(M_prime_hash, M_prime, (8U + SHA256_HASH_LEN + salt_length) * 8U);
	if (result != SUCCESS) {
		return result;
	}
	...
	if (salt_length > 0U) {
		(void)memcpy(data_block + padding_string_len + 1U, salt_to_use, salt_length);
	}

	/* Generating dbMask = MGF(M_prime_hash, data_block_length)
	 *maskedDB = data_block XOR dbMask */
	result = Mask_Generation(dbMask, data_block_length, M_prime_hash, SHA256_HASH_LEN);
	if (result != SUCCESS) {
		return result;
	}
			   ...
			   size_t input_length, size_t expected_salt_length)
{
	...
	result = Mask_Generation(dbMask, data_block_length, signature_hash, SHA256_HASH_LEN);
	if (result != SUCCESS) {
		return result;
	}
	...
	if (expected_salt_length > 0U) {
		(void)memcpy(M_prime + 8U + SHA256_HASH_LEN, data_block + padding_string_len + 1U,
