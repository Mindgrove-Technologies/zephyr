 ...
 * Name of the file             : rsa_driver.c
 * Brief Description of file    : Contains the driver routines for rsa driver.
 * Name of Author               : Siddhanth Ramani
 * Email ID                     : siddhanth@mindgrovetech.in

 Copyright (C) 2025 Mindgrovetech Pvt Ltd. All rights reserved.
 ***************************************************************************/
/**
  @file rsa_driver.c
  @brief Contains the driver routines for rsa driver.
*/
...
#define R2_STR "10000000000000000000000000000000000000000000000"    \
 ...
 */
static void do_checks_rsa(unsigned char *mod_text,
                          int input_len_bits,
                          int exp_len_bits,
                          int mod_len_bits,
                          struct_rsa_padding rsa_padding) {
    // Length of input, exp, mod should be less than 2048 bits
    if ((input_len_bits > 2048) ||
        (exp_len_bits > 2048)   ||
        (mod_len_bits > 2048)) {
 ...
 */
static void get_rsa_output(volatile uint64_t *rsa_output_reg_64,
                           uint64_t *rsa_output) {
    // rsa_output += 32;
    for (int output_registers_index = 0;
         output_registers_index < 32;
...
static void swap_endian_in_place(uint64_t *num) {
    *num = ((*num >> 56) & 0x00000000000000FFULL) |
...
uint32_t RSA_Run(uint8_t *output, uint8_t *input, uint8_t *exp, uint8_t *mod)
{
    ...
    BigNum_Write_Unsigned_Bin(&r2modn_result, \
                              (uint8_t *)r2modn + (256 - r2modn_size), 256);

    uint8_t* src[4] = {input, exp, mod, (uint8_t*)r2modn};
    uint64_t* dest[4] = {&rsa_instance->RSA_INPUT, \
                         &rsa_instance->RSA_EXP, \
                         &rsa_instance->RSA_MOD, \
                         &rsa_instance->RSA_RSqrMODN};
