...
#include "gpio_seciot.h"

#include <zephyr/drivers/gpio/gpio_utils.h>


#define DT_DRV_COMPAT seciot_gpio

typedef void (*seciot_cfg_func_t)(void);

typedef void (*gpio_seciot_cfg_func_t)(void);

typedef struct gpio_seciot_regs_t
{
...
struct gpio_seciot_config{
/* gpio_driver_config needs to be first */
    struct gpio_driver_config common;
    uintptr_t gpio_base_addr;
    uint32_t gpio_irq_base;
    gpio_seciot_cfg_func_t gpio_cfg_func;
    uint32_t gpio_mode;
};

struct gpio_seciot_data {
...
#define DEV_GPIO_CFG(dev)						\
	((const struct gpio_seciot_config * const)(dev)->config)
#define DEV_GPIO(dev)							\
	((volatile struct gpio_seciot_regs_t *)(DEV_GPIO_CFG(dev))->gpio_base_addr)
#define DEV_GPIO_DATA(dev)				\
	((struct gpio_seciot_data *)(dev)->data)


int gpio_seciot_init(const struct device *dev){
    
    volatile struct gpio_seciot_regs_t *gpio = DEV_GPIO(dev);
    const struct gpio_seciot_config *cfg = DEV_GPIO_CFG(dev);
...
static int gpio_seciot_pin_configure (const struct device *dev, 
                        gpio_pin_t pin, 
                        gpio_flags_t flags){
...
static int gpio_seciot_pin_get_raw(const struct device *dev,
                    gpio_pin_t pin)
{
    volatile struct gpio_seciot_regs_t *gpio = DEV_GPIO(dev);
    return gpio->data;
...
static int gpio_seciot_pin_set_raw(const struct device *dev,
                    gpio_pin_t pin)
{
    volatile struct gpio_seciot_regs_t *gpio = DEV_GPIO(dev);   
    const struct gpio_seciot_config *cfg = DEV_GPIO_CFG(dev);
...
static int gpio_seciot_pin_toggle(const struct device *dev,
                    gpio_pin_t pin)
{
    // printf("toggle pin\n");
    volatile struct gpio_seciot_regs_t *gpio = DEV_GPIO(dev);
...
static int gpio_seciot_pin_clear_raw(const struct device *dev,
                    gpio_pin_t pin)
{
    volatile struct gpio_seciot_regs_t *gpio = DEV_GPIO(dev);   
...
static inline unsigned int gpio_seciot_pin_irq(unsigned int base_irq, int pin)
{
...
static int gpio_seciot_irq_handler(const struct device *dev)
{
    struct gpio_seciot_data *data = DEV_GPIO_DATA(dev);
    volatile struct gpio_seciot_regs_t *gpio_reg = DEV_GPIO(dev);
    const struct gpio_seciot_config *cfg = DEV_GPIO_CFG(dev); 
...
static void gpio_seciot_isr(const struct device *dev)
{
    printf("Entered GPIO ISR()\n");
}

static int gpio_seciot_pin_interrupt_configure(const struct device *dev, 
                                                gpio_pin_t pin, 
                                                gpio_flags_t flag)
{
    volatile struct gpio_seciot_regs_t *gpio_reg = DEV_GPIO(dev);
    ...
    irq_enable(gpio_seciot_pin_irq(cfg->gpio_irq_base, pin));

    return 0;
}

static const struct gpio_driver_api gpio_seciot_driver = {
    .pin_configure              = gpio_seciot_pin_configure,
    .port_get_raw               = gpio_seciot_pin_get_raw,   
    .port_set_bits_raw          = gpio_seciot_pin_set_raw,    
    .port_clear_bits_raw        = gpio_seciot_pin_clear_raw,
    .port_toggle_bits           = gpio_seciot_pin_toggle, 
    .pin_interrupt_configure    = gpio_seciot_pin_interrupt_configure,  
};

// #define		IRQ_INIT(n)					\
// IRQ_CONNECT(n+32,   \
// 		1,		\
// 		gpio_seciot_irq_handler,    \
// 		NULL,				\
// 		0)

static void gpio_seciot_cfg(uint32_t gpio_pin){
...
static const struct gpio_seciot_config gpio_seciot_config0 ={
    // .common = {
    //     .port_pin_mask  = GPIO_PORT_PIN_MASK_FROM_DT_INST(0),
    // },
    .gpio_base_addr     = GPIO_START,
    .gpio_irq_base      = GPIO_IRQ_BASE,
    .gpio_cfg_func      = gpio_seciot_cfg,
    .gpio_mode          = DT_PROP(DT_NODELABEL(gpio0), config_gpio)
};

static struct gpio_seciot_data gpio_seciot_data0 ={
...
DEVICE_DT_INST_DEFINE(inst, \
                gpio_seciot_init,  \
                NULL, \
                &gpio_seciot_data0, &gpio_seciot_config0, \
                PRE_KERNEL_1, CONFIG_GPIO_INIT_PRIORITY, \
                &gpio_seciot_driver); 

// IRQ_CONNECT(1, 1, gpio_seciot_isr, DEVICE_DT_GET(DT_NODELABEL(gpio0)), 0));
