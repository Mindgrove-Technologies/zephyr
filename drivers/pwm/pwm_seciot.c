...
#define DT_DRV_COMPAT seciot_pwm
...
/*Data structure*/
struct pwm_seciot_data {};

/*Zephyr configuration structure*/
struct pwm_seciot_cfg {
	uint32_t base;
	uint32_t f_sys;
	uint32_t cmpwidth;
 ...
 */
static int pwm_seciot_init(const struct device *dev)
{
	const struct pwm_seciot_cfg *cfg = dev->config;
	char *pwm_no;
	pwm_no = dev->name;
 ...
 * @param[Out] No output parameter
 */
static int pwm_seciot_set_cycles(const struct device *dev, uint32_t channel,uint32_t period_cycles, uint32_t pulse_cycles, pwm_flags_t flags)
{
	const struct pwm_seciot_cfg *cfg = dev->config;
...
static const struct pwm_driver_api pwm_seciot_api = {
	.set_cycles = pwm_seciot_set_cycles,
};
// .cmpwidth = DT_INST_PROP(n, seciot_compare_width), \

#define PWM_SECIOT_INIT(n)	\
	static struct pwm_seciot_data pwm_seciot_data_##n;	\
	static const struct pwm_seciot_cfg pwm_seciot_cfg_##n = {	\
			.base = PWM_START_##n ,	\
...
DT_INST_FOREACH_STATUS_OKAY(PWM_SECIOT_INIT)
