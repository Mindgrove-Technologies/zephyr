 ...
 */
static inline int configure_control(bool update, pwm_interrupt_modes interrupt_mode, bool change_output_polarity)
{
	int value = 0x0;
...
static int pwm_mindgrove_set_cycles(const struct device *dev, uint32_t channel,uint32_t period_cycles, uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	{
	case PWM_0: {
		static const int db_config0[] = DT_PROP(DT_NODELABEL(pwm0), db_configure);
		deadband_delay = db_config0[0];
		control_reg = db_config0[1];
		prescale = db_config0[2];
		break; }
	case PWM_1: {
		static const int db_config1[] = DT_PROP(DT_NODELABEL(pwm1), db_configure);
		deadband_delay = db_config1[0];
		control_reg = db_config1[1];
		prescale = db_config1[2];
		break; }
	case PWM_2: {
		static const int db_config2[] = DT_PROP(DT_NODELABEL(pwm2), db_configure);
					    ...
					    uint64_t *cycles)
{
	...
	/* Verify the channel is valid for your hardware */
	if (channel > 13) {
		return -EINVAL;
	}

	/* Return the system frequency defined in your config */
	*cycles = (uint64_t)cfg->f_sys;
