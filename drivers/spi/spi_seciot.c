...
#define DT_DRV_COMPAT seciot_spi

#define LOG_LEVEL CONFIG_SPI_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(spi_seciot);

#include <soc.h>
#include <stdbool.h>
#include <zephyr/sys/util.h>
#include<zephyr/kernel.h>
#include "spi_seciot.h"
#include <stdio.h>
...
static int spi_seciot_transceive(const struct device *dev,
			  ...
			  const struct spi_buf_set *rx_bufs)
{
  ...
  k_mutex_lock(&(((struct spi_seciot_cfg*)(dev->config))->mutex),K_FOREVER);
  ...
  k_mutex_unlock(&(((struct spi_seciot_cfg*)(dev->config))->mutex));
  return 0;  
}

int sspi_seciot_init(const struct device *dev)
{ 
  // printk("init\n");
  int spi_base;
  char *spi_inst; 
  spi_inst = dev->name;
  struct spi_seciot_cfg *confg = (struct spi_seciot_cfg *)dev->config;
  printk("SPI: %s\n", spi_inst);
  spi_number = spi_inst[6] - '0';
  gpio_pin_configure_dt(&(((struct spi_seciot_cfg*)(dev->config))->ncs),1);
  printk("SPI NUMBER: %d\n", spi_number);
...
static int spi_seciot_release(const struct device *dev,
		       const struct spi_config *config)
{
...
static struct spi_driver_api spi_seciot_api = {
	.transceive = spi_seciot_transceive,
	.release = spi_seciot_release,
};

#define SPI_INIT(n)	\
  static struct spi_seciot_data spi_seciot_data_##n = { \
    SPI_CONTEXT_INIT_LOCK(spi_seciot_data_##n, ctx), \
    SPI_CONTEXT_INIT_SYNC(spi_seciot_data_##n, ctx), \
    SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(n), ctx)	\
  }; \
  static struct spi_seciot_cfg spi_seciot_cfg_##n = { \
