...
#include "aes128_cbc_mmt.h"
#include "aes192_cbc_mmt.h"
#include "aes256_cbc_mmt.h"
...
#define RUN_CBC_BATCH(type, arr, cnt, label)                            \
    do {                                                                \
        ...
        for (uint32_t i = 0; i < (cnt); i++) {                        \
            ...
            uint8_t iv[16];                                            \
            if ((tv->pt_len % 16) != 0) {                              \
                printk("[" label " %03d] SKIP (unaligned)\n", i);     \
                continue;                                              \
            }                                                          \
...
int test_aes_cbc_mmt(void)
{
    ...
    RUN_CBC_BATCH(CBC_MMT_128_KAT_Test_Vectors,
                  cbc_mmt_128_kat_vectors,
                  cbc_mmt_128_kat_vectors_count,
                  "CBC MMT 128");

    RUN_CBC_BATCH(CBC_MMT_192_KAT_Test_Vectors,
                  cbc_mmt_192_kat_vectors,
                  cbc_mmt_192_kat_vectors_count,
                  "CBC MMT 192");

    RUN_CBC_BATCH(CBC_MMT_256_KAT_Test_Vectors,
                  cbc_mmt_256_kat_vectors,
                  cbc_mmt_256_kat_vectors_count,
                  "CBC MMT 256");
