...
#include "sha256_mmt_test_vectors_03.h"   /* defines test_vectors, mmt_03_kat_vectors_count,
                                            sha256_multi_kat_03 */
...
int test_sha256_mmt_03(void)
{
    ...
    int total_pass = 0;
    int count = (int)mmt_03_kat_vectors_count;

    for (int v_idx = 0; v_idx < count; v_idx++) {
        const sha256_multi_kat_03 *tv = &test_vectors[v_idx];
        uint8_t output[SHA256_HASH_LEN] = {0};
        struct hash_ctx ctx;
        struct hash_pkt pkt;

        printk("\n=== MMT03 Vector %d (%u bits) ===\n",
        ...
        if (memcmp(output, tv->output_hash, SHA256_HASH_LEN) == 0) {
            printk("MMT03 Vector %d: PASS\n", v_idx);
            total_pass++;
        } else {
            printk("MMT03 Vector %d: FAIL\nExpected: ", v_idx);
