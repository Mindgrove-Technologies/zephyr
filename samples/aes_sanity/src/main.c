...
static int aes_ecb_multivector(void)
{
    ...
    for (uint32_t i = 0; i < kat_vectors_count; i++) {
        ...
        /* Step 1: isolate ECB */
        if (tv->mode != CRYPTO_CIPHER_MODE_ECB)
            continue;
...
static int aes_kat_multimode(void)
{
    ...
    for (uint32_t i = 0; i < kat_vectors_count; i++) {
        ...
        /* ---- FIX: Map custom mode → Zephyr mode ---- */
...
static int aes_ecb_mmt_validator(void) {
    printk("=== AES ECB MMT Multi-block Test START ===\n");
    ...
    for (uint32_t i = 0; i < mmt_vectors_count; i++) {
        ...
        /* Only test ECB vectors */
        if (tv->mode != AES_ECB)
            continue;

        uint32_t key_bits = tv->key_len * 8;
        ...
        for (uint32_t block = 0; block < block_count; block++) {
            ...
            ASSERT_OK(cipher_block_op(&ctx_enc, &pkt),
                      "ECB encrypt failed");
        }
        ...
        ASSERT_MEM_EQ(decrypted_text,
                      tv->plaintext,
                      tv->pt_len,
                      "ECB decryption mismatch");

        printk("Decryption PASS\n");
    }

    printk("=== AES ECB MMT Multi-block Test COMPLETE ===\n");
...
static int aes_cbc_mmt_multivector(void)
{
    printk("=== AES CBC MMT Multi-block Test START ===\n");
    ...
    for (uint32_t i = 0; i < mmt_vectors_count; i++) {
        ...
        printk("\n====================================\n");
        printk("CBC MMT Vector %u (Key %u-bit)\n",
               i, tv->key_len * 8);
        printk("====================================\n");
        ...
        ASSERT_MEM_EQ(cipher_text, tv->ciphertext,
                      tv->ct_len,
                      "CBC encryption mismatch");
        ...
        ASSERT_MEM_EQ(decrypted_text, tv->plaintext,
                      tv->pt_len,
                      "CBC decryption mismatch");

        printk("Decryption PASS\n");
    }

    printk("=== AES CBC MMT Multi-block Test COMPLETE ===\n");
    return 0;
}

static int aes_ctr_mmt_multivector(void)
{
    printk("=== AES CTR MMT Multi-block Test START ===\n");
    ...
    for (uint32_t i = 0; i < mmt_vectors_count; i++) {
        ...
        printk("\n====================================\n");
        printk("CTR MMT Vector %u (Key %u-bit)\n",
               i, tv->key_len * 8);
        printk("====================================\n");
        ...
        ASSERT_MEM_EQ(decrypted_text, tv->plaintext,
                      tv->pt_len,
                      "CTR decryption mismatch");

        printk("Decryption PASS\n");
    }

    printk("=== AES CTR MMT Multi-block Test COMPLETE ===\n");
    return 0;
}
...
void main(void)
{
    ...
    if(aes_ecb_mmt_validator()){
        printk("AES ECB MMT KAT FAILED\n");
    } else {
        printk("AES ECB MMT KAT PASSED\n");
    }

     if(aes_cbc_mmt_multivector()){
        printk("AES CBC MMT KAT FAILED\n");
    } else {
        printk("AES CBC MMT KAT PASSED\n");
    }

    if(aes_ctr_mmt_multivector()){
        printk("AES CTR MMT KAT FAILED\n");
    } else {
        printk("AES CTR MMT KAT PASSED\n");
    }
    ...
    //  if(aes_cbc_mmt_multivector()){
    //      printk("AES CBC MMT MULTIVECTOR FAILED\n");
    //  } else {
    //      printk("AES CBC MMT MULTIVECTOR PASSED\n");
    //  }
    //  if(aes_ctr_mmt_multivector()){
    //      printk("AES CTR MMT MULTIVECTOR FAILED\n");
    //  } else {
    //      printk("AES CTR MMT MULTIVECTOR PASSED\n");
    //  }
