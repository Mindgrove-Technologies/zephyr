...
#include "oaep_decrypt_kat_testvectors_02.h"
...
int test_rsa_oaep_decrypt_02(void)
{
    ...
    uint16_t count = oaep_dec_kat_vectors_count_02;

    for (uint16_t i = 0; i < count; i++) {
        const OAEP_Decrypt_Test_Vectors_02 *tv = &oaep_decrypt_kat_vectors_02[i];
        ...
        if (!valid) {
            if (tv->expected_result == 'F') {
                printk("[OAEP DEC 02 %03d] PASS (c>=n, expected fail)\n", i);
                passed++;
            } else {
                printk("[OAEP DEC 02 %03d] FAIL (c>=n but expected pass)\n", i);
                failed++;
            }
            continue;
        }
        ...
        if (tv->expected_result == 'P') {
            ...
            if (!ok)
                printk("[OAEP DEC 02 %03d] FAIL (expected pass)\n", i);
        } else {
            ok = (ret != 0) ||
                 (pkt.out_len != (tv->input_len_bits / 8)) ||
                 (memcmp(tv->input, decrypted, pkt.out_len) != 0);
            if (!ok)
                printk("[OAEP DEC 02 %03d] FAIL (expected fail)\n", i);
        }

        printk("[OAEP DEC 02 %03d] %s\n", i, ok ? "PASS" : "FAIL");
        ok ? passed++ : failed++;
    }

    printk("OAEP DECRYPT 02: %d Passed, %d Failed\n\n", passed, failed);
    return failed ? -EIO : 0;
}